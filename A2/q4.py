...
if __name__ == "__main__":
    ...
    # sample1 = load_color_image("colourTemplate.png")
    # sample2 = load_color_image("colourSearch.png")
    # print(sample1.shape)
    # print(sample2.shape)

    # # Load sample1 and find keypoints, descriptors
    # kp1, des1, result1 = sift_color(sample1)
    # save_image("sample1_color_kp.jpg", result1)

    # # Load sample2 and find keypoints, descriptors 
    # kp2, des2, result2 = sift_color(sample2)
    # save_image("sample2_color_kp.jpg", result2)
    ...
    # test1 = cv2.drawKeypoints(sample1, [i[0] for i in r], sample1, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    # test2 = cv2.drawKeypoints(sample2, [i[1] for i in r], sample2, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    
    # # Concatenate two images
    # test2 = add_zeros(test1, test2)
    # test = concatenate(test1, test2)
    # horizontal_shift = test1.shape[1]
    # for i in r:
    #     r0 = int(i[0].pt[1])
    ...
    #     rr, cc, val = line_aa(r0, c0, r1, c1)
    #     test[rr, cc] = np.array([0, 255, 0])
    # save_image("color_l2norm.jpg", test)